...
<template>
  <div v-if="executor" class="flex items-center gap-2.5">
    <img
      ...
      :alt="name"
      class="h-10 w-10 shrink-0 rounded-full object-cover ring-2 ring-sky-500/40"
    />
    <span
      v-else
      class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-surface-muted text-sm font-semibold text-text-primary dark:text-zinc-100"
      aria-hidden="true"
    >
      {{ initial }}
    </span>
